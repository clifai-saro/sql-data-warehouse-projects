...
BEGIN
	...
	BEGIN TRY
		...
		SELECT 
			...
			prd_st_dt,
			LEAD(prd_st_dt) OVER (PARTITION BY prd_key ORDER BY prd_st_dt)-1 AS prd_end_dt  --Calculate end date as one day before the next start date
		...
		CASE WHEN UPPER(TRIM(gen)) IN ('F', 'FEMALE') THEN 'Femaile'
